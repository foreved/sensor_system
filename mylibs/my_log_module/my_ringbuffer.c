...
void my_ringbuffer_consume(My_RingBuffer_Obj *this, int num)
{
    ...
    if (num > free_size)
        my_ringbuffer_free(this, num - free_size);


    this->head = (this->head + num) & (MY_RINGBUFFER_SIZE - 1);
}
...
void my_ringbuffer_write(My_RingBuffer_Obj *this, uint8_t *buffer, int num)
{
    ...
    if (tmp < num)
    {
        ...
        memcpy(this->buffer, buffer + tmp, num - tmp);
    }
    else
    {
        memcpy(&this->buffer[this->head], buffer, num);
    }
...
int my_ringbuffer_get_continue_valid_size(My_RingBuffer_Obj *this)
{
    if (this->head >= this->tail)
        ...
        return MY_RINGBUFFER_SIZE - this->tail;
}
